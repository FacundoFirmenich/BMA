...
def object_gate(period_state: str, source_volume_state: object) -> str | None:
    if period_state != "VALID":
        return period_state
    if not str(source_volume_state).startswith("VERIFIED_"):
        return "NOT_ESTIMABLE_SOURCE_VOLUME_CONFLICT"
    return None
...
def main() -> None:
    ...
    for obj in offer["objects"]:
        ...
        gate_state = object_gate(period_state, obj.get("location_volume_state"))
        matches = [] if gate_state is not None else [
        ...
        objects.append({
            ...
            "no_match_state": gate_state or ("NOT_ESTIMABLE_NO_PRIOR_COMPARABLE_BASE_CELL" if not matches else None),
        })
    freeze = {
        "schema_version": "bpm.rmk.event-freeze.v0.6",
