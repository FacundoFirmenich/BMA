...
const [inputPath, offerPath, outputPath, sourceSha256, eventDate, protocolDate, sourceBytesText, quarantinedResultUrl = "", freezePath = ""] = process.argv.slice(2);
...
for (const row of values.slice(headerIndex + 1)) {
  ...
  if (product !== object.product) {
    failures.push({ object_id: objectId, frozen_product: object.product, result_product: product, state: "RESULT_PRODUCT_MISMATCH" });
  }
  ...
  const numericPrice = typeof rawPrice === "number"
    ? rawPrice
    : Number(String(rawPrice ?? "").trim().replace(",", "."));
  const priceEstimable = Number.isFinite(numericPrice);
  if (!priceEstimable) {
    failures.push({ object_id: objectId, raw_price: rawPrice, state: "NOT_ESTIMABLE_PRICE_ENCODING" });
  }
  rows.push({
    object_id: objectId,
    product,
    ...
    effective_weighted_price_eur_m3: priceEstimable ? numericPrice : null,
    price_state: priceEstimable ? "ESTIMABLE_EXACT_DESTINATION" : "NOT_ESTIMABLE_PRICE_ENCODING",
  });
}
...
const result = {
  schema_version: "bpm.rmk.result-structured.v0.3",
